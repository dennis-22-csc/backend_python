...
class ClientPasswordTest(unittest.TestCase):
    ...
    def test_client_password_endpoint_for_auth_code(self, case_name, pay_load, expected_code, expected_reason, expected_message, mock_validate_auth_code, mock_has_expired, mock_delete_obj):
        ...
        if case_name == "no_auth":
            ...
            response = self.app.post('/v1/oauth/client_password', headers=self.header, json=pay_load)
    ...
    @patch('v1.oauth.views.index.has_expired')
    @patch('v1.oauth.views.index.validate_auth_code')
    def test_client_password_endpoint_correct_input(self, pay_load, expected_code, expected_length, mock_validate_auth_code, mock_has_expired, mock_client_exist, mock_hash_password,mock_update_obj):
        """Test endpoint for correct input."""
        auth_obj_mock = Mock(spec=AuthCode)
        mock_validate_auth_code.return_value = auth_obj_mock
        mock_has_expired.return_value = False
        mock_client_exist.return_value = True 
        mock_hash_password.decode.return_value = "yyyyeeeeffff"
        mock_update_obj.return_value = MagicMock(id=1, password='yyyyeeeeffff')
        response = self.app.post('/v1/oauth/client_password', json=pay_load, headers=self.header)
        self.assertEqual(response.status_code, expected_code)
        response_data_dict = json.loads(response.data.decode('utf-8'))
